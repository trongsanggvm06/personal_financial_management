...
// A transaction as returned by the API (amount sign encodes income/expense).
...
test('filter transactions by type=expense', async () => {
  const res = await request(app)
    ...
    .set('Authorization', `Bearer ${token}`);
  assert.equal(res.status, 200);
  assert.ok(res.body.transactions.every((t: ApiTransaction) => t.amount < 0));
});
...
test('delete transaction removes it', async () => {
  ...
  const list = await request(app)
    ...
    .set('Authorization', `Bearer ${token}`);
  assert.ok(!list.body.transactions.some((t: ApiTransaction) => t.id === txId));
});
...
test('budget: upsert + reports over-budget spending', async () => {
  ...
  const res = await request(app)
    ...
    .set('Authorization', `Bearer ${token}`);
  const b = res.body.budgets.find((x: ApiBudget) => x.category === 'Ăn uống');
