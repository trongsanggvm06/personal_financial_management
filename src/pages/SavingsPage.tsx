...
function GoalModal({ open, onClose, onSaved, initial }: GoalModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError((err as Error).message || 'Không lưu được mục tiêu.');
    } finally {
      setSaving(false);
    }
  };
...
function DepositModal({ open, goal, onClose, onSaved }: DepositModalProps) {
  const [amount, setAmount] = useState('');
  const [direction, setDirection] = useState<'in' | 'out'>('in'); // in | out
  const [error, setError] = useState('');
  const [saving, setSaving] = useState(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError((err as Error).message || 'Không thực hiện được.');
    } finally {
      setSaving(false);
    }
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) setError((err as Error).message || 'Không tải được mục tiêu.');
      } finally {
