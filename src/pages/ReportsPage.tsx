...
function StatBox({ icon: Icon, label, value, tone }: StatBoxProps) {
  const toneClass =
...
export default function ReportsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) setError((err as Error).message || 'Không tải được báo cáo.');
      } finally {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      setError((err as Error).message || 'Không tải được file CSV.');
    } finally {
        ...
        <div className="flex flex-wrap items-end gap-2">
          <div>
            ...
            <input
              ...
              value={range.from}
              max={range.to}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setRange((r) => ({ ...r, from: e.target.value }))}
          ...
          <div>
            ...
            <input
              ...
              value={range.to}
              min={range.from}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setRange((r) => ({ ...r, to: e.target.value }))}
              className="rounded-lg border border-zinc-800 bg-zinc-950/60 px-3 py-2 text-xs font-medium text-zinc-100 outline-none transition focus:border-indigo-500/60 [color-scheme:dark]"
            />
          </div>
          <button
