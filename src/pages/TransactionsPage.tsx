...
import type { Transaction, OutletContext } from '../types';
...
export default function TransactionsPage() {
  const { openNewTransaction } = useOutletContext<OutletContext>();
  const {
    activeAccount,
    accountTransactions,
    accounts,
  ...
  const [typeFilter, setTypeFilter] = useState<'all' | 'income' | 'expense'>('all'); // all | income | expense
  const [categoryFilter, setCategoryFilter] = useState('All');

  const [editing, setEditing] = useState<Transaction | null>(null); // transaction being edited
  const [deleting, setDeleting] = useState<Transaction | null>(null); // transaction being deleted
